...
for d in [TERMUX_HOME, TERMUX_TMP, TERMUX_VAR, TERMUX_STATIC]:
                ...
                if ch == '\\r':
                    ...
                    if cmd == "help":
                        ...
                        await websocket.send_text(
                            ...
                            "  Or open /static/ghost_worker.html in a browser tab.\\r\\n"
                        )
                    elif cmd == "clear":
                        await websocket.send_text("\\x1b[2J\\x1b[H")
                    elif cmd == "exit":
...
async def root():
    with open(os.path.join(TERMUX_STATIC, "index.html")) as f:
        return f.read()
...
async def execute_command(request: CommandRequest, req: Request = None):
    ...
    try:
        ...
        result = subprocess.run(
            request.command, shell=True, cwd=cwd, env=env,
...
async def worker_ws(websocket: WebSocket):
    ...
    logger.info(f"[Worker {worker_id}] Connected from {worker_registry[worker_id]['ip']}")
    try:
        while True:
            msg = await websocket.receive_text()
            try:
                ...
                logger.warning(f"[Worker {worker_id}] Invalid JSON")
                continue
            msg_type = data.get("type")
            if msg_type in ("inference_result", "inference_response"):
                jid = data.get("job_id") or data.get("task_id")
                if jid:
                    inference_results[jid] = data.get("result", "")
                    logger.info(f"[Worker {worker_id}] Returned result for job {jid}")
            elif msg_type in ("heartbeat", "worker_registration"):
                worker_registry[worker_id]["last_seen"] = time.time()
                if data.get("hardware"):
                    worker_registry[worker_id]["hardware"] = data["hardware"]
                if data.get("worker_id"):
                    worker_registry[worker_id]["client_id"] = data["worker_id"]
    except WebSocketDisconnect:
...
async def terminal_page():
    with open(os.path.join(TERMUX_STATIC, "terminal.html")) as f:
        return f.read()


@app.get("/files", response_class=HTMLResponse)
async def file_manager_page():
    with open(os.path.join(TERMUX_STATIC, "file_manager.html")) as f:
        return f.read()


@app.get("/ghost", response_class=HTMLResponse)
async def ghost_worker_page():
    with open(os.path.join(TERMUX_STATIC, "ghost_worker.html")) as f:
        return f.read()
